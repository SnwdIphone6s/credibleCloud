        ...
        <div class="arrow_header">    
          <div v-for="(data, index) in host_info_list" :key="index" class="arrow">
            <div class="arrow_1">
              ...
              <el-radio :label="data.id"><span style="display: none">{}</span></el-radio>
            </el-radio-group>
            </div>
            <div class="arrow_2">
              <p>
          ...
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
            <el-form-item label="供应商" prop="provider">
              <el-select v-model="ruleForm.provider" placeholder="请选择">
                <el-option v-for="(item,index) in allData" :label="item.name" :value="item.id" :key="item.id"></el-option>
              </el-select>
            </el-form-item>
  ...
  data() {
   ...
   watch: {
    'ruleForm.ip': function(curVal, oldVal) {
      ...
      // 实时把非数字的输入过滤掉
      this.ruleForm.ip = curVal.match(/\d/ig) ? curVal.match(/\d/ig).join('') : ''
    }
  },
  created() {
    this.getList()
    ...
    getList(){
        facilitatorList().then(data => {
          ...
          allData.forEach(data=>{
            ...
            if( v.service_provider == data.id){
                 v.service_provider_name = data.name
                 return v
            }
           })
          })
